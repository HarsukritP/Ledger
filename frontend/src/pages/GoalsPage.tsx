...
const FEASIBILITY_STYLES = {
  on_track: { label: "On Track", className: "bg-income/15 text-income border border-income/20" },
  at_risk: { label: "At Risk", className: "bg-warning/15 text-warning border border-warning/20" },
  behind: { label: "Behind", className: "bg-danger/15 text-danger border border-danger/20" },
} as const;
  ...
  const handleCreate = async () => {
    ...
    try {
      await api.goals.create({
        name: newName, target_amount: parseFloat(newTarget),
        target_date: newDate || new Date(Date.now() + 180 * 86400000).toISOString().split("T")[0],
      });
      setShowCreate(false); setNewName(""); setNewTarget(""); setNewDate("");
      loadGoals();
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between">
        ...
        <button onClick={() => setShowCreate(true)} className="flex items-center gap-1.5 rounded-full bg-gold px-4 py-2 text-xs font-semibold text-black transition-all hover:brightness-110">
      ...
      {goals.length === 0 && !showCreate ? (
        <div className="card p-8 text-center">
          <p className="text-sm text-text-muted">No goals yet. Create your first savings goal to get started.</p>
          <button onClick={() => setShowCreate(true)} className="mt-4 rounded-full bg-gold px-6 py-2 text-xs font-semibold text-black">Create Goal</button>
        </div>
      ) : (
            ...
            return (
              ...
              >
                <div className="flex items-center gap-5">
                  <GoalRing progress={progress} size={80} strokeWidth={5} color="#22C55E">
                    <span className="font-mono text-sm font-semibold text-north-star">{Math.round(progress * 100)}%</span>
                  </GoalRing>
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <h3 className="text-base font-semibold text-text-primary">{goal.name}</h3>
                      <span className={cn("rounded-full px-2 py-0.5 text-[10px] font-medium", fStyle.className)}>{fStyle.label}</span>
                    </div>
                    <div className="mt-1 flex items-baseline gap-1">
                      <MoneyText value={goal.currentAmount} className="text-lg text-north-star" />
                      <span className="text-sm text-text-muted">/ ${goal.targetAmount.toLocaleString()}</span>
                    </div>
                    <p className="mt-1 text-xs text-text-secondary">
                      ${Math.round(goal.monthlyContribution)}/mo needed &middot;{" "}
                      {goal.targetDate ? new Date(goal.targetDate + "T12:00:00").toLocaleDateString("en-US", { month: "short", year: "numeric" }) : "No deadline"}
                    </p>
                  </div>
      ...
      <AnimatePresence>
        {showCreate && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            className="fixed inset-0 z-50 flex items-center justify-center bg-overlay p-4" onClick={() => setShowCreate(false)}>
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }}
              ...
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => setShowCreate(false)}><X size={20} className="text-text-muted" /></button>
              </div>
              <div className="mt-4 space-y-3">
                <input type="text" placeholder="Goal name (e.g. Japan Trip)" value={newName} onChange={(e) => setNewName(e.target.value)}
                ...
                <input type="number" placeholder="Target amount ($)" value={newTarget} onChange={(e) => setNewTarget(e.target.value)}
                  className="w-full rounded-xl border border-border bg-base px-4 py-2.5 text-sm text-text-primary placeholder:text-text-muted focus:border-gold/50 focus:outline-none" />
                ...
                <button onClick={handleCreate} disabled={creating || !newName || !newTarget}
      ...
      <AnimatePresence>
        {selectedGoal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            className="fixed inset-0 z-50 flex items-center justify-center bg-overlay p-4" onClick={() => setSelectedGoal(null)}>
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }}
              onClick={(e) => e.stopPropagation()} className="card w-full max-w-lg p-6">
              <div className="flex items-center justify-between">
                <h2 className="text-xl font-bold text-text-primary">{selectedGoal.name}</h2>
                <button onClick={() => setSelectedGoal(null)}><X size={20} className="text-text-muted" /></button>
              </div>
              <div className="mt-6 flex justify-center">
                <GoalRing progress={selectedGoal.targetAmount > 0 ? selectedGoal.currentAmount / selectedGoal.targetAmount : 0} size={160} strokeWidth={8} color="#22C55E">
                  <div className="text-center">
                    <span className="font-mono text-2xl font-semibold text-north-star">
                      {Math.round(selectedGoal.targetAmount > 0 ? (selectedGoal.currentAmount / selectedGoal.targetAmount) * 100 : 0)}%
                    </span>
              ...
              {editing ? (
                <div className="mt-6 space-y-3">
                  ...
                  <div className="flex gap-2">
                    <button onClick={handleSaveEdit} disabled={saving} className="flex-1 rounded-full bg-gold py-2 text-xs font-semibold text-black disabled:opacity-50">
                      {saving ? "Saving..." : "Save Changes"}
                    </button>
                    <button onClick={() => setEditing(false)} className="rounded-full border border-border px-4 py-2 text-xs text-text-muted">Cancel</button>
                  </div>
                </div>
              ) : (
                <>
                  <div className="mt-6 space-y-3">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="text-text-primary">${selectedGoal.currentAmount.toLocaleString()} / ${selectedGoal.targetAmount.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                    ...
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="text-text-primary">
                        {selectedGoal.targetDate ? new Date(selectedGoal.targetDate + "T12:00:00").toLocaleDateString("en-US", { month: "long", year: "numeric" }) : "Not set"}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="mt-4 flex gap-2">
                    <button onClick={startEdit} className="flex flex-1 items-center justify-center gap-1.5 rounded-full border border-border py-2 text-xs font-medium text-text-secondary hover:bg-surface-hover">
                      <Pencil size={12} /> Edit Goal
                    </button>
                    <button onClick={() => handleDelete(selectedGoal.id)} className="flex-1 rounded-full border border-danger/30 py-2 text-xs font-medium text-danger hover:bg-danger/5">
                      Delete Goal
                    </button>
                  </div>
                </>
